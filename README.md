...
- **多节点下载测速(`--speedtest`)**:从本机出口顺序下载 Cachefly/Linode 东京/Linode 美西/ThinkBroadband 等节点测下载速率(Mbps),每节点上限 50MB/10s;速率着色;配置文件 `[[speedtest]]` 可自定义节点(测国内三网点);因耗流量较大不含在 --all 内
...
ipano --speedtest      # 多节点下载测速(顺序下载,耗流量较大,不含在 --all)
ipano --no-color       # 关闭彩色
...
| P15 | **多节点下载测速**(4 节点 HTTP 下载,节点可配置,`--speedtest`)| ✅ |
